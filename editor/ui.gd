...
func initialize(on_tool_selected : Callable, on_snap_increment_set : Callable, on_snap_increment_doubled_or_halved : Callable, on_local_transform_active_set : Callable, on_snapping_active_set : Callable):
	UI.b_drag = %Button
	UI.b_move = %Button2
	UI.b_rotate = %Button3
	UI.b_scale = %Button4
	...
	UI.b_spawn.pressed.connect(on_tool_selected.bind(UI.b_spawn))
...
static func select_tool(
	...
	if selected_parts_array.size() > 0 and not selected_tool_handle_array.is_empty():
